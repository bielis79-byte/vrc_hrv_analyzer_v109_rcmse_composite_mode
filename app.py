...
st.set_page_config(page_title="VRC / HRV RRi Analyzer Pro v10.9", layout="wide")
...
PARAM_GROUPS = {
    ...
    "Control Kubios": ["Entropy_lambda", "Entropy_m", "Entropy_r_factor", "Entropy_SD_ms", "Entropy_r_ms", "Entropy_N", "DFA_alpha1_range", "DFA_alpha2_range", "RQA_threshold_SD", "RQA_emb_dim", "MSE_zero_policy"],
}
...
def _sample_entropy_counts(x, m=2, r=None):
    """
...
def _sample_entropy_core(x, m=2, r=None, zero_policy="nan"):
    """
    SampEn con parámetros Kubios:
    - m = 2
    - r = 0.2 x SD
    - sin self-matches

    zero_policy:
    - "nan": si A=0 devuelve NaN.
    - "half_count": si A=0 usa A=0.5.
    - "one_count": si A=0 usa A=1.0.
    """
    b, a = _sample_entropy_counts(x, m=m, r=r)
...
def sample_entropy_common(rr_entropy, m=KUBIOS_ENTROPY_M, r_factor=KUBIOS_ENTROPY_R_FACTOR, r_reference=None, zero_policy=None):
    """
    ...
    return _sample_entropy_core(x, m=m, r=r, zero_policy=zero_policy)
...
def _sample_entropy_counts_from_series(x, m=2, r=None):
    """
    Alias explícito para conteos SampEn de una serie ya coarse-grained.
    """
    return _sample_entropy_counts(x, m=m, r=r)


def rcmse_common(rr_entropy, scales=KUBIOS_MSE_MAX_SCALE, m=KUBIOS_ENTROPY_M, r_factor=KUBIOS_ENTROPY_R_FACTOR, r_reference=None):
    """
    ...
    for scale in range(1, int(scales) + 1):
        if scale <= 1:
            out[f"MSE{scale}"] = _sample_entropy_core(x, m=m, r=r, zero_policy="nan")
            continue
        ...
        for offset in range(scale):
            ...
            B, A = _sample_entropy_counts_from_series(cg, m=m, r=r)
...
def rcmse_diagnostic_rows(rr_entropy, scales=20, m=2, r_factor=0.2, r_reference=None):
    """
...
def mse_common(rr_entropy, scales=KUBIOS_MSE_MAX_SCALE, m=KUBIOS_ENTROPY_M, r_factor=KUBIOS_ENTROPY_R_FACTOR, r_reference=None, zero_policy="nan"):
    """
    MSE v10.6.
    ...
    for scale in range(1, int(scales) + 1):
        ...
        if len(cg) <= m + 2:
            ...
            out[f"MSE{scale}"] = _sample_entropy_core(cg, m=m, r=r, zero_policy=zero_policy)

    # MSE1 se mantiene igual a SampEn clásica principal.
    out["MSE1"] = _sample_entropy_core(x, m=m, r=r, zero_policy=zero_policy)
    return out
...
def mse_metrics(rr, scales=20, max_scale=None, m=2, r=0.2, zero_policy=None):
    """
    ...
    return mse_common(rr, scales=scales, m=m, r_factor=r, r_reference=rr, zero_policy=zero_policy)
...
def sample_entropy_diagnostic_rows(rr_entropy, scales=20, m=2, r_factor=0.2, r_reference=None):
    """
    ...
    for scale in range(1, int(scales) + 1):
        ...
        B, A = _sample_entropy_counts(cg, m=m, r=r)
...
def calculate_all(rr, include_rqa=True, include_hvg=False, mse_zero_policy=None):
    """
...
def calculate_record(rr, windows, active_phases, min_rr, include_rqa, include_hvg=False, mse_zero_policy=None):
    rows, segments, valid = [], {}, {}
    ...
    for ph in PHASES:
        ...
        if valid[ph]:
            res = calculate_all(seg, include_rqa=include_rqa, include_hvg=include_hvg, mse_zero_policy=mse_zero_policy)
            res["Fase"] = ph
            rows.append(res)
...
def _single_record_report(record_name, metrics_df, windows, rr=None):
    ...
    if any(c in metrics_df.columns for c in MSE_COLUMNS):
        ...
        if rr is not None:
            ...
            lines.append("")
            lines.append("| Fase | Escala | N | r ms | B | A | A/B | Clásico | A0=0.5 | A0=1.0 | RCMSE | Estado |")
            lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
...
st.title("VRC / HRV RRi Analyzer Pro v10.9")
...
for rec, data in record_data.items():
    ...
    df, segs, valid = calculate_record(data["rr"], w, active_phases, min_rr, include_rqa, include_hvg=include_hvg, mse_zero_policy=st.session_state.get("mse_zero_policy", "nan"))
